...
namespace DynamicExtensions
{
    public class ObjectMerger : IObjectMerger
    {
        ...
        public static Type GetCachedOrCreateType(Type[] types, Type tOut)
        {
        ...
        static readonly IDictionary<CtorKey, object> ctorsCache = new ConcurrentDictionary<CtorKey, object>();
        ...
        public static Func<object[], TOut> GetCachedOrCreateCtor<TOut>(Type[] types) where TOut : class
        {
            ...
            if (!ctorsCache.TryGetValue(ctorKey, out object ctorResult))
            {
                lock (sync)
                {
                    if (!ctorsCache.TryGetValue(ctorKey, out ctorResult))
                    {
                        ValidateCtorCreation(tOut, types);
                        var tResult = GetCachedOrCreateType(types, tOut);
                        ctorResult = CreateConstructor<TOut>(tResult, types);
                        ctorsCache.Add(ctorKey, ctorResult);
                    }
                }
            }
        ...
        private static void ValidateCtorCreation(Type tOut, Type[] types)
        {
